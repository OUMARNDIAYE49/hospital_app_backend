...
const specialiteController = {
  ...
  createSpecialite: async (req, res) => {
    ...
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
        return res.status(400).json({ message: `La spécialité "${nom}" existe déjà.` });
      }
      console.error(error);
      return res.status(500).json({ message: "Erreur lors de la création de la spécialité", error });
    }
  },
  ...
  getSpecialiteById: async (req, res) => {
    ...
    try {
      ...
      console.error(error);
      return res.status(500).json({ message: "Erreur lors de la récupération de la spécialité", error });
    }
  },
  ...
  getSpecialites: async (req, res) => {
    try {
      ...
      console.error(error);
      return res.status(500).json({ message: "Erreur lors de la récupération des spécialités", error });
    }
  },
  ...
  updateSpecialite: async (req, res) => {
    ...
    try {
      ...
      console.error(error);
      return res.status(500).json({ message: "Erreur lors de la mise à jour de la spécialité", error });
    }
  },
  ...
  deleteSpecialite: async (req, res) => {
    ...
    try {
      ...
      console.error(error);
      return res.status(500).json({ message: "Erreur lors de la suppression de la spécialité", error });
    }
  },
};
