...
export const createPatient = [
  ...
  body('prenom')
    ...
    .isLength({ max: 100 }).withMessage('Le prénom doit avoir au maximum 100 caractères'),
  body('dateNaissance')
    .notEmpty().withMessage('La date de naissance est obligatoire') // Vérifie que le champ est présent
    .bail() // Arrête ici si la validation précédente échoue
    .isISO8601().withMessage('La date de naissance doit être une date valide au format AAAA-MM-JJ')
    .toDate(),
  body('telephone')
    .optional()
